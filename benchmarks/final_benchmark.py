...
def run_rs_batch(pos_np, batch_np, cells_np, cutoff, parallel=True):
    neighborlist_rs.build_neighborlists_batch(pos_np, batch_np, cells=cells_np, cutoff=cutoff, parallel=parallel)
...
def run_torch_cluster_multi_filtered(pos_t, batch_t, cutoffs, device):
    max_cut = max(cutoffs)
    min_cut = min(cutoffs)
    edge_index = torch_cluster.radius_graph(pos_t, r=max_cut, batch=batch_t, max_num_neighbors=100000)
...
def benchmark_single_systems(data_dir):
    ...
    for cutoff in CUTOFFS:
        ...
        for name, f in files:
            ...
            t, std = benchmark_func(lambda: run_matscipy(atoms_boxed, cutoff), "Matscipy")
            if t:
            ...
            if torch_cluster:
                if is_isolated:
                    ...
                    t, std = benchmark_func(lambda: run_torch_cluster(pos_t, batch_t, cutoff, "cpu"), "TC CPU")
                    if t:
                        print(f"{ ' TorchCluster':<20} | CPU   | {t:<10.2f} | {std:<10.2f}")
                    
                    if torch.cuda.is_available():
                        device = "cuda"
                        pos_gpu = pos_t.to(device)
                        batch_gpu = batch_t.to(device)
                        t, std = benchmark_func(lambda: run_torch_cluster(pos_gpu, batch_gpu, cutoff, device), "TC GPU")
                        if t:
                            print(f"{ ' TorchCluster':<20} | GPU   | {t:<10.2f} | {std:<10.2f}")
                else:
                    print(f"{ ' TorchCluster':<20} | ---   | {'N/A (No PBC support)':<10} | {'':<10}")

            if torch_nl:
                # Must use boxed version for cell
                pos_t = torch.tensor(atoms_boxed.get_positions(), dtype=torch.float32)
                batch_t = torch.zeros(len(atoms), dtype=torch.long)
                cell_t = torch.tensor(atoms_boxed.get_cell().array, dtype=torch.float32).unsqueeze(0)
                t, std = benchmark_func(lambda: run_torch_nl(pos_t, cell_t, batch_t, cutoff, "cpu"), "TNL CPU")
                if t:
                ...
                if torch.cuda.is_available():
                    ...
                    t, std = benchmark_func(lambda: run_torch_nl(pos_gpu, cell_gpu, batch_gpu, cutoff, device), "TNL GPU")
                    if t:
...
def benchmark_multi_cutoff(data_dir):
    ...
    # ASE Filtered
    t, std = benchmark_func(lambda: run_ase_multi_filtered(atoms_boxed, MULTI_CUTOFFS), "ASE Filtered")
    if t:
        print(f"{ ' ASE (Filtered)':<20} | CPU   | {t:<10.2f} | {std:<10.2f}")

    # Matscipy Filtered
    t, std = benchmark_func(lambda: run_matscipy_multi_filtered(atoms_boxed, MULTI_CUTOFFS), "Matscipy Filtered")
    if t:
        print(f"{ ' Matscipy (Filt)':<20} | CPU   | {t:<10.2f} | {std:<10.2f}")

    # Freud Filtered
    t, std = benchmark_func(lambda: run_freud_multi_filtered(atoms_boxed, MULTI_CUTOFFS), "Freud Filtered")
    if t:
        print(f"{ ' Freud (Filt)':<20} | CPU   | {t:<10.2f} | {std:<10.2f}")
    
    # Vesin Filtered
    t, std = benchmark_func(lambda: run_vesin_multi_filtered(atoms_boxed, MULTI_CUTOFFS), "Vesin Filtered")
    if t:
...
def benchmark_batching(data_dir):
    ...
    print(f"{'Batch Size':<10} | {'Library':<20} | {'Device':<5} | {'Time (ms)':<10} | {'Throughput':<15}")
    print("-" * 80)
    ...
    for b_size in BATCH_SIZES:
        ...
        neighborlist_rs.set_num_threads(8)
        t, std = benchmark_func(lambda: run_rs_batch(pos_np, batch_ids, cells_np, cutoff), "RS Batch")
        if t: 
        ...
        if torch_cluster:
            ...
            if t:
                thru = b_size / (t / 1000.0)
                print(f"{b_size:<10} | {'TorchCluster':<20} | CPU   | {t:<10.2f} | {thru:<15.2f}")
            ...
            if torch.cuda.is_available():
                ...
                if t:
                    thru = b_size / (t / 1000.0)
                    print(f"{b_size:<10} | {'TorchCluster':<20} | GPU   | {t:<10.2f} | {thru:<15.2f}")
        
        if torch_nl:
            t, std = benchmark_func(lambda: run_torch_nl(pos_t, cells_t, batch_t, cutoff, "cpu"), "TNL CPU")
            if t:
                thru = b_size / (t / 1000.0)
                print(f"{b_size:<10} | {'TorchNL':<20} | CPU   | {t:<10.2f} | {thru:<15.2f}")
            ...
            if torch.cuda.is_available():
                ...
                t, std = benchmark_func(lambda: run_torch_nl(pos_gpu, cells_gpu, batch_gpu, cutoff, device), "TNL GPU")
                if t:
                    thru = b_size / (t / 1000.0)
                    print(f"{b_size:<10} | {'TorchNL':<20} | GPU   | {t:<10.2f} | {thru:<15.2f}")
