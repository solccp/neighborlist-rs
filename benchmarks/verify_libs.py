...
def verify():
    ...
    # Compare RS vs ASE
    assert len(results['ase'][0]) == len(results['rs'][0]), f"Count mismatch: ASE={len(results['ase'][0])}, RS={len(results['rs'][0])}"
    
    np.testing.assert_array_equal(results['ase'][0], results['rs'][0], err_msg="Source indices mismatch")
    np.testing.assert_array_equal(results['ase'][1], results['rs'][1], err_msg="Target indices mismatch")
