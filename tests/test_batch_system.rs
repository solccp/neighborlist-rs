...
fn test_build_neighborlists_batch_mixed() {
    ...
    let result = build_neighborlists_batch(&positions, &batch, cutoff, Some(&cells_data), true).unwrap();
    ...
    // Check if (0,1) exists
    let idx0 = src.iter().position(|&x| x == 0).expect("Edge 0->1 not found");
    ...
    // Check if (2,3) exists
    let idx1 = src.iter().position(|&x| x == 2).expect("Edge 2->3 not found");
