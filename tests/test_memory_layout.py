...
def test_batch_memory_layout():
    ...
    assert not positions_nc.flags['C_CONTIGUOUS']
    assert not batch_nc.flags['C_CONTIGUOUS'] # Might be depending on numpy version, but let's try
    ...
    try:
        res = neighborlist_rs.build_neighborlists_batch(positions_nc, batch_nc, None, cutoff)
